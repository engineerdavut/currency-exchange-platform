...
export default function TransactionsPage() {
  ...
  const [currencyFilter, setCurrencyFilter] = useState<CurrencyType | 'ALL'>("ALL");
  const { transactions: allTransactions, isLoading: isLoadingTransactions, error: transactionsError } = useTransactions();
  ...
  useEffect(() => {
    if (!isLoadingTransactions) {
      ...
      setFilteredTransactions([]); // Yüklenirken boşalt
   }
  ...
  // Oturum kontrolü için yükleme durumu
    ...
    <div className="max-w-4xl mx-auto">
      <Card className="mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
          <h2 className="text-2xl font-bold text-gray-800 mb-4 sm:mb-0">Transaction History</h2>
        ...
        {transactionsError && <ErrorMessage message={transactionsError} className="mb-4" />}        
        {isLoadingTransactions ? (
          ...
          <div className="text-center py-8 text-gray-500">
            {currencyFilter === 'ALL' ? 'No transactions found.' : 'No transactions found for this currency.'}
          </div>
        )}
      </Card>
    </div>
