...
export default function AccountPage() {
  ...
  useEffect(() => {
    // Auth yüklenmediyse veya kullanıcı giriş yapmamışsa login'e yönlendir
  ...
  const handleModalSubmit = async (amountValue: string) => {
    ...
    // Çekim işleminde bakiye kontrolü yap
    if (modalType === 'withdraw') {
      const currentWallet = wallet.find(w => w.currencyType === selectedCurrency);
      if (!currentWallet || numericAmount > currentWallet.balance) {
        setError(`Insufficient balance. Your current balance is ${formatCurrency(currentWallet?.balance || 0, selectedCurrency)}`);
        return;
      }
    }
    ...
    try {
      ...
      closeModal();
      if (isAuthenticated && !isAuthLoading) { // Tekrar fetch et
        const response = await accountApi.getWallet();
        setWallet(response.data);
      }
      // window.location.reload(); // Bu kötü bir pratik, state yönetimi ile çözülmeli
    } catch (error: unknown) {
      let errorMessage = 'Deposit/Withdraw failed. Please try again.';
      if (error instanceof AxiosError) { // Axios hatası kontrolü
        errorMessage = error.response?.data?.message || error.message;
      } else if (error instanceof Error) { // Genel hata kontrolü
    ...
    <div className="max-w-4xl mx-auto p-4">
      ...
      {wallet.length > 0 ? (
        <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {wallet.map((acc) => (
            <Card key={acc.accountId} className="hover:shadow-lg transition-shadow">
              {/* Card içeriği aynı kalabilir */}
              <div className="flex flex-col h-full">
                <div className="mb-4">
                  <h3 className="text-xl font-semibold text-blue-600">{acc.currencyType}</h3>
                  <p className="text-gray-700 mt-2">
                    <span className="font-medium">Balance:</span> {formatCurrency(acc.balance, acc.currencyType)}
                  </p>
                </div>
                ...
                <div className="mt-auto flex space-x-2">
                  <Button
                    ...
                    className="flex-1"
                    disabled={isSubmitting}>Deposit</Button>
                  <Button
                    variant="danger"
